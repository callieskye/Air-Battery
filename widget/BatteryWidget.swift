...
// Was hardcoded "com.lihaoyun6.AirBattery" (the original developer's Bundle Identifier) in
// all four "is the main app running" checks below. Since this patched build's Bundle
// Identifier is com.callieskye.AirBattery, that check could never match the real running
// app, so `mainApp` was always false here - which is exactly what drives the widget's
// "AirBattery is not running / Launch the app to make the widget work" placeholder message,
// regardless of whether the app was actually running. This one constant is now the single
// place that needs to change if the Bundle Identifier ever changes again.
let mainAppBundleID = "com.callieskye.AirBattery"
