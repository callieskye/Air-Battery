...
func getPowerState() -> iBattery {
    ...
    if let internalBattery = internalFinder.getInternalBattery() {
        if let level = internalBattery.charge {
            ...
            // isLowPowerModeEnabled requires macOS 12+, always true now that the minimum
...
func getHeadphoneModel(_ model: String) -> String {
    ...
    case "2013":
        return "Airpods 3"
    case "201b", "2019":
        // Fixes issue #166 (AirPods 4 showing AirPods Pro icons for both the case and the
        // earbuds). The model string passed in here is always built via
        // String(format: "%02x%02x", ...) in getAirpods() below, which always produces
        // LOWERCASE hex digits. This case was written as "201B" (uppercase B) - Swift string
        // matching is case-sensitive, so the actual value ("201b") never matched, and the ANC
        // AirPods 4 model silently fell through to the "Headphones" default below. That
        // unrecognized model then also fell through the icon-lookup switch's own default arm
        // in getDeviceIcon() (Supports.swift), which resolves to the AirPods Pro icon -
        // exactly the wrong icon reported. The non-ANC AirPods 4 ID ("2019") has no letters in
        // it, so it was matching correctly the whole time; only the ANC variant was broken.
        ///201b: ANC
        ///2019: no ANC
        return "Airpods 4"
    case "2014", "2024":
...
func getDeviceIcon(_ d: Device) -> String {
    ...
    case "Remote":
        // Fixes issue #85 (feature request): icon for a Siri Remote, if one is ever detected
        // via the generic HID battery path (see the "Remote" classification added in
        // MagicBattery.swift's readMagicBattery()).
    ...
    case "Pencil":
        return "pencil"
    case "ap_pod_right":
        // A newer over-ear model (e.g. "AirPods Max 2") won't match any case in the model
        // table below since it broadcasts BLE bytes this reverse-engineered table doesn't
        // know about yet, and the earbud fallback icon is doubly wrong for something that
        // isn't even an earbud. The device's broadcast name reliably contains "Max" for any
        // AirPods Max variant, so use that as a higher-priority check.
        if d.deviceName.lowercased().contains("max") { return "airpodsmax" }
        if let model = d.deviceModel {
            switch model {
            case "Airpods":
                return "airpod.right"
            ...
            case "Beats Studio Buds+":
                return "beats.studiobud.right"
            default:
                // Unknown/unrecognized model (e.g. a newer AirPods Pro 2 firmware whose
                // advertisement bytes don't match a known case) used to silently fall back
                // to the plain "Airpods" icon here, which reads as "this is a regular
                // AirPods", not "unrecognized". Match ap_pod_all's default instead, which
                // is a less misleading guess since Pro-style buds are the common case.
                return "airpodpro.right"
            }
        }
        return "airpodpro.right"
    ...
    case "macbook", "macbookpro", "macbookair", "macbookneo":
        // deviceType for a Mac comes from getMacDeviceType(), which just lowercases and
        // strips spaces from system_profiler's marketing name (e.g. "MacBook Air" ->
        // "macbookair"). "MacBook Neo" (Apple's entry-level laptop below the MacBook Air,
        // launched March 2026, model identifier Mac17,5) becomes "macbookneo" that way —
        // a deviceType this switch never listed, so it fell all the way through to the
        // unrelated final "questionmark.circle.fill" default instead of showing a laptop
        // icon at all.
